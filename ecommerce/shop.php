...
if ($_GET) {
    if (isset($_GET['brand'])) {
        if (is_numeric($_GET['brand'])) {
            ...
            if ($brandObject->getBrandById()->get_result()->num_rows == 1) {
                $productObject->setBrand_id($_GET['brand']);
                $products = $productObject->productsByBrand()->get_result()->fetch_all(MYSQLI_ASSOC);
            } else {
        ...
        if (is_numeric($_GET['subcategory'])) {
            ...
            if ($subcategoryObject->getSubcategoryById()->get_result()->num_rows == 1) {
                $productObject->setSubcategory_id($_GET['subcategory']);
                $products = $productObject->productsBySubcategory()->get_result()->fetch_all(MYSQLI_ASSOC);
            } else {
                include "layouts/errors/404.php";
                die;
            }
    ...
    $products = $productObject->read()->get_result()->fetch_all(MYSQLI_ASSOC);
}
?>
